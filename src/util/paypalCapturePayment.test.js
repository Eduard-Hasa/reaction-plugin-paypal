...
import paypalCapturePayment from "./paypalCapturePayment.js";
...
test("should call paypalApi.methods.captureCharge with the proper parameters and return saved = true", async () => {
  const paymentMethod = {
    processor: "paypal",
  ...
  nock("https://api.paypal.com:443")
    ...
    .reply(200, paypalCaptureResult);

  const result = await paypalCapturePayment(mockContext, paymentMethod);
...
test("should should return an error if transactionId is not available", async () => {
  ...
  const result = await paypalCapturePayment(mockContext, paymentMethod);
  expect(result.error.type).toBe("paypalConnectionError");
});
